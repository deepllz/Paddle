...
from .parallel_base import ParallelModelBase, ParallelOptimizer


class ShardedDataParallel(ParallelModelBase):
    """
    ShardedDataParallel converts a single card model to a distrubuted data parallel model

    Args:
        model (paddle.nn.Layer): A single card model.
    ...
    def __init__(
        self, model, level=None, scaler=None, offload=False, exclude_layer=None
    ):
...
def sharded_data_parallel(
    ...
    sdp_model = ShardedDataParallel(
        model, level, scaler, offload, exclude_layer
    )
    sdp_optimizer = ParallelOptimizer(optimizer, sdp_model)
    ...
    return sdp_model, sdp_optimizer
