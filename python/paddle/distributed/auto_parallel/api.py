...
def shard_tensor(
    ...
    if (
        ...
        and isinstance(data, paddle.Tensor)
        and data.is_dist()
    ):
        return data
...
class DistModel:
    ...
    Args:
        ...
        input_spec(list): inputs and labels spec with format [inputs_spec, labels_spec]. if input_spec is None, the inputs and labels spec will be inferred from loader.
        loader(paddle.io.DataLoader): The data loader used in dygraph mode,
            used to infer input_spec if it is None.
    ...
    def __init__(
        ...
        if input_spec is not None:
            inputs_spec, labels_spec = input_spec
        else:
...
def to_static(
    ...
    Args:
        ...
        input_spec(list): inputs and labels spec with format [inputs_spec, labels_spec]. if input_spec is None, it will be inferred from loader.
        loader(paddle.io.DataLoader): The data loader used in dygraph mode,
            used to infer inputs_spec and labels_spec if input_spec is None.
        loss(Loss|Callable|None, optional): The loss function for training
            or evaluating the model. Can be a `paddle.nn.Layer` instance or
    ...
    dist_model = DistModel(layer, input_spec, loader, loss, optimizer, strategy)
...
class ShardDataloader:
    ...
    Args:
        ...
        The input corresponding to input_keys[i] is on meshes[i].
        if outputs is a list, no need to pass it, and the i'th input is on meshes[i].
...
def shard_dataloader(
    ...
    """
    Convert the dataloader to a ShardDataloader. See ShardedDataloader for details

    Args:
        dataloader (paddle.io.DataLoader): The dataloader to be sharded.
        meshes (Union(list, tuple)): The mesh list of the dataloader. Identify which mesh the input is on.
        if len(meshes) == 1, all the inputs are on the same mesh.
        input_keys Union(list, tuple): The input keys of the outputs of dataloader if it is a dict.
        The input corresponding to input_keys[i] is on meshes[i].
        if outputs is a list, no need to pass it, and the i'th input is on meshes[i].
        shard_dims (Union[list, tuple, str, int]): The mesh dimension to shard the dataloader.
